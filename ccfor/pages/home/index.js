...
Page({
    ...
    data: {
      ...
      catgs: [
        ...
        { name: "健身", val: "4", bar: [{ name: "签到训练", url: "../sports/sign/index" }, { name: "动作库", url: "../sports/catg/index" }, { name: "健身达人", url: "" }]}, 
    ...
    onLoad: function (option) {
      ...
      wx.request({
        url: 'https://www.ccco.club/api/Article/getlist',
        success: (res) => {
          console.log(res)
          res.data.data.forEach(item => {
            item.photo ="https://www.ccco.club"+item.photo.replace('\\','\/')
          ...
          //      }
          // })
          //this.setData({ articles: res.data.data, catgs:this.data.catgs })
    ...
    touchend(e) {
      console.log(e)
      if (e.changedTouches[0].pageX - this.data.touchStartX < - 100 && this.data.catgIndex <this.data.catgs.length - 1) {
        this.setData({ catgIndex: this.data.catgIndex + 1 })
        console.log("向左滑")
        this.tapCatg(null)
      }
      if (e.changedTouches[0].pageX - this.data.touchStartX > 100 && this.data.catgIndex >= 1){
        this.setData({ catgIndex: this.data.catgIndex - 1})
        console.log("向右滑")
        this.tapCatg(null)
      }
